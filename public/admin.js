...
const $=s=>document.querySelector(s);let token=sessionStorage.getItem('adminToken');let currentPage=1;let departments=[];let staff=[];let dashboardCache=null;let currentUser=null;let executiveDepartmentId='';let governanceMode='categories';let governanceEditing=null;let complaintPendingDelete=null;let smartGeoMap=null;let smartGeoMarkers=null;let smartGeoMarkerById=new Map();let smartGeoResizeTimer=null;let selectedMapMonth='all';let mobileMapPage=1;const mobileMapPageSize=4;const reportInsightPages={topCategory:1,category:1,department:1};
...
function isSystemAdmin(){return ['admin','dev'].includes(currentUser?.role)}
...
function roleLabel(role){return({admin:'ผู้ดูแลระบบ',dev:'ผู้พัฒนาระบบ',supervisor:'หัวหน้าหน่วยงาน',officer:'เจ้าหน้าที่',executive:'ผู้บริหาร',exclusive:'ผู้บริหาร'})[role]||role}
function appView(user){currentUser=user;const isOfficer=user.role==='officer';const showUserInsights=isSystemAdmin();$('#loginPanel').classList.add('hidden');$('#appShell').classList.remove('hidden');$('#staffName').textContent=user.displayName||user.username;$('#staffRole').textContent=roleLabel(user.role);$('#todayLabel').textContent=new Intl.DateTimeFormat('th-TH',{dateStyle:'full'}).format(new Date());$('#topCategoriesCard').classList.toggle('hidden',isOfficer);$('#departmentWorkloadCard').classList.toggle('hidden',isOfficer);$('#userStatusCard').classList.toggle('hidden',!showUserInsights);$('#topUserDepartmentsCard').classList.toggle('hidden',!showUserInsights)}
function logout(){token=null;executiveDepartmentId='';sessionStorage.removeItem('adminToken');loginView()}
function openLogoutConfirmation(){const dialog=$('#logoutConfirmDialog');if(!dialog.open)dialog.showModal()}
function closeLogoutConfirmation(){const dialog=$('#logoutConfirmDialog');if(dialog.open)dialog.close('cancel')}
function confirmLogout(){closeLogoutConfirmation();logout()}
...
function applyRoleVisibility(){
  const isElevated=isSystemAdmin()||currentUser?.role==='supervisor'||isExecutive();
  const isAdmin=isSystemAdmin();
  const isDev=currentUser?.role==='dev';
  if((!isAdmin&&governanceMode==='users')||(!isDev&&governanceMode==='staffProfiles'))governanceMode='categories';
  ...
  document.querySelector('.governance-tab[data-governance="staffProfiles"]')?.classList.toggle('hidden',!isDev);
...
function renderDashboard(){renderKpis();renderTrend('#trendChart',dashboardCache.monthlyTrend);renderDonut();renderUserUsage();renderUrgent();if(currentUser?.role!=='officer'){renderPagedInsight('#categoryBars','#topCategoryPagination',dashboardCache.categoryBreakdown||[],'topCategory',{pageSize:5,showPercentage:false});renderBars('#departmentBars',dashboardCache.departmentBreakdown)}$('#recentTable').innerHTML=tableHtml(dashboardCache.recent,true)}
function renderKpis(){const s=dashboardCache.summary;const items=[['▤','เรื่องทั้งหมด',s.total,'ข้อมูลสะสม','#0b695b','#e4f3ef'],['◷','รับเรื่องเดือนนี้',s.this_month,'เดือนปัจจุบัน','#3c7ce8','#e8efff'],['⚙','กำลังดำเนินการ',s.in_progress,'อยู่ระหว่างทำงาน','#8e59d8','#f0e8fb'],['✓','เสร็จสิ้น',s.completed,'ปิดงานแล้ว','#18a575','#e1f5ed'],['!','เกินกำหนด',s.overdue,'ต้องเร่งติดตาม','#d84c4c','#fde9e8'],['◆','ความสำคัญสูง',s.high_priority,'High / Urgent','#ef9b2d','#fff0dc']];const usage=dashboardCache.backendUsage;if(isSystemAdmin()&&usage){items.push(['♟','ผู้ใช้งาน Backend',usage.active_users,`เปิดใช้งานจาก ${usage.total_users} บัญชี`,'#7b3fa0','#f3eafb'],['⌂','หน่วยงานเข้าใช้มากที่สุด',usage.department_name||'-',usage.department_name?`${usage.active_user_count} คนใน ${usage.period_days} วัน`:'ยังไม่มีผู้เข้าใช้ในช่วง 30 วัน','#9b5fc0','#f5edfa',true])}const grid=$('#kpiGrid');grid.classList.toggle('system-admin-kpis',isSystemAdmin()&&Boolean(usage));grid.innerHTML=items.map(([i,l,v,t,c,soft,textValue])=>`<article class="v3-kpi" style="--kpi:${c};--kpi-soft:${soft}"><div class="v3-kpi-icon">${i}</div><span>${escapeHtml(l)}</span><strong class="${textValue?'kpi-text-value':''}">${escapeHtml(v??0)}</strong><small>${escapeHtml(t)}</small></article>`).join('')}
...
function renderBars(sel,data=[],suffix='',showPercentage=false,options={}){const totalData=options.totalData||data;const totalValues=totalData.map(x=>Math.max(0,Number(x.value)||0));const values=data.map(x=>Math.max(0,Number(x.value)||0));const max=Math.max(1,...totalValues);const sum=totalValues.reduce((total,value)=>total+value,0);const scale=showPercentage?Math.max(1,sum):max;const rankOffset=Number(options.rankOffset)||0;$(sel).innerHTML=data.map((x,i)=>{const value=values[i];const percentage=sum?value/sum*100:0;const percentageLabel=percentage.toLocaleString('th-TH',{minimumFractionDigits:percentage%1===0?0:1,maximumFractionDigits:1});const valueLabel=`${x.value}${suffix?` ${suffix}`:''}${showPercentage?` (${percentageLabel}%)`:''}`;return `<div class="metric-item ${showPercentage?'insight-metric-item':''}"><div class="metric-label"><span class="metric-name">${showPercentage?`<i class="metric-rank">${rankOffset+i+1}</i>`:''}${escapeHtml(x.label)}</span><b class="metric-value">${escapeHtml(valueLabel)}</b></div><progress class="metric-progress metric-tone-${(rankOffset+i)%6}" value="${value}" max="${scale}" aria-label="${escapeHtml(x.label)} ${escapeHtml(valueLabel)}"></progress></div>`}).join('')||'<p class="muted">ยังไม่มีข้อมูล</p>'}
function renderPagedInsight(sel,pagerSel,data=[],pageKey,options={}){const configuredPageSize=Number(options.pageSize)||0;const totalPages=configuredPageSize?Math.max(1,Math.ceil(data.length/configuredPageSize)):(data.length>1?2:1);const pageSize=configuredPageSize||Math.max(1,Math.ceil(data.length/totalPages));const page=Math.min(totalPages,Math.max(1,reportInsightPages[pageKey]||1));const showPercentage=options.showPercentage!==false;reportInsightPages[pageKey]=page;const start=(page-1)*pageSize;renderBars(sel,data.slice(start,start+pageSize),'',showPercentage,{totalData:data,rankOffset:start});const pager=$(pagerSel);pager.innerHTML=Array.from({length:totalPages},(_,index)=>`<button type="button" class="insight-page-button ${page===index+1?'active':''}" data-page="${index+1}" aria-label="หน้าที่ ${index+1}" ${page===index+1?'aria-current="page"':''}>${index+1}</button>`).join('');pager.querySelectorAll('.insight-page-button').forEach(button=>button.onclick=()=>{reportInsightPages[pageKey]=Number(button.dataset.page);renderPagedInsight(sel,pagerSel,data,pageKey,options)})}
...
function renderUserUsage(){if(!isSystemAdmin())return;const usage=dashboardCache.backendUsage;if(!usage)return;const labels={recent_login:'เข้าใช้ใน 30 วัน',no_recent_login:'ยังไม่เข้าใช้ใน 30 วัน',inactive:'ระงับบัญชี'};const colors={recent_login:'#18a575',no_recent_login:'#ef9b2d',inactive:'#8a9692'};const data=usage.user_status_breakdown||[];const total=data.reduce((sum,item)=>sum+Number(item.value),0);let cursor=0;const stops=data.filter(item=>Number(item.value)>0).map(item=>{const start=cursor;cursor+=Number(item.value)/Math.max(1,total)*360;return `${colors[item.label]||'#8a9692'} ${start}deg ${cursor}deg`}).join(',');const donut=$('#userStatusDonut');donut.style.background=stops?`conic-gradient(${stops})`:'#eef3f1';donut.innerHTML=`<div class="donut-center"><div><strong>${total}</strong><span>บัญชี</span></div></div>`;$('#userStatusLegend').innerHTML=data.map(item=>`<div class="legend-row"><span><i class="legend-color" style="background:${colors[item.label]||'#999'}"></i>${escapeHtml(labels[item.label]||item.label)}</span><b>${escapeHtml(item.value)}</b></div>`).join('');renderBars('#userDepartmentBars',usage.department_login_breakdown||[],'คน')}
...
async function openCase(id){
  ...
  const assignSection=canManageAssignment?`<section class="drawer-section"><h3>มอบหมาย</h3><div class="drawer-form"><label>หน่วยงาน<select id="assignDepartment" ${canChangeDepartment?'':'disabled'}>${depOptions}</select>${canChangeDepartment?'':'<small class="muted">เฉพาะ Admin และ Supervisor เท่านั้นที่เปลี่ยนหน่วยงานได้</small>'}</label><label>เจ้าหน้าที่ผู้รับผิดชอบ<select id="assignStaffProfile"><option value="">กำลังโหลดรายชื่อ…</option></select><small class="muted">รายชื่อจะแสดงตามหน่วยงานที่เลือก</small></label><div class="two"><label>ความสำคัญ<select id="assignPriority"><option value="low">ต่ำ</option><option value="normal">ปกติ</option><option value="high">สูง</option><option value="urgent">เร่งด่วน</option></select></label><label>กำหนดเสร็จ<input id="assignDue" type="datetime-local"></label></div><label>หมายเหตุ<input id="assignNote" placeholder="รายละเอียดการมอบหมาย"></label><button id="assignButton" class="v3-primary">บันทึกการมอบหมายและแจ้ง LINE</button></div></section>`:'';
  ...
  if(canManageAssignment){$('#assignPriority').value=c.priority||'normal';if(c.due_at)$('#assignDue').value=new Date(c.due_at).toISOString().slice(0,16);await loadAssignmentStaff(c.department_id,c.assigned_staff_profile_id);$('#assignDepartment').onchange=()=>loadAssignmentStaff($('#assignDepartment').value,null);$('#assignButton').onclick=()=>assignCase(c.id)}
  if(canEditStatus){
...
async function loadAssignmentStaff(departmentId,selectedId=null){const select=$('#assignStaffProfile');if(!select)return;if(!departmentId){select.innerHTML='<option value="">ยังไม่มอบหมายเจ้าหน้าที่</option>';select.disabled=true;return}select.disabled=true;select.innerHTML='<option value="">กำลังโหลดรายชื่อ…</option>';try{const result=await api(`/api/admin/assignment-staff?departmentId=${encodeURIComponent(departmentId)}`);select.innerHTML='<option value="">ยังไม่มอบหมายเจ้าหน้าที่</option>'+result.data.map(profile=>`<option value="${profile.id}" ${profile.id===selectedId?'selected':''}>${escapeHtml(profile.full_name)}${profile.position_title?` • ${escapeHtml(profile.position_title)}`:''}${profile.phone?` • ${escapeHtml(profile.phone)}`:''}</option>`).join('');select.disabled=false}catch(error){select.innerHTML='<option value="">โหลดรายชื่อไม่สำเร็จ</option>';show('#pageAlert',error.message)}}
...
async function assignCase(id){
  try{
    ...
    show('#pageAlert',result.message||'บันทึกการมอบหมายเรียบร้อย',result.data?.lineNotified===false?'error':'success');
    closeDetailDrawer();
    await boot();
  }catch(e){
...
function renderReports(){const s=dashboardCache.summary;const data=[['เรื่องทั้งหมด',s.total],['เดือนนี้',s.this_month],['กำลังดำเนินการ',s.in_progress],['เสร็จสิ้น',s.completed],['เฉลี่ยวันดำเนินการ',s.avg_days]];$('#reportSummary').innerHTML=data.map(([l,v])=>`<div><strong>${v||0}</strong><span>${l}</span></div>`).join('');renderPagedInsight('#reportCategory','#reportCategoryPagination',dashboardCache.categoryBreakdown||[],'category');renderPagedInsight('#reportDepartment','#reportDepartmentPagination',dashboardCache.departmentBreakdown||[],'department');renderTrend('#reportTrend',dashboardCache.monthlyTrend)}
...
function parseAuditDetail(detail){if(typeof detail!=='string')return detail&&typeof detail==='object'?detail:{};try{const parsed=JSON.parse(detail);return parsed&&typeof parsed==='object'?parsed:{value:detail}}catch{return {value:detail}}}
function auditNoteHtml(detail){const value=parseAuditDetail(detail);const deletionReason=String(value.deletionReason||'').trim();return deletionReason?`<div class="audit-delete-note">${escapeHtml(deletionReason)}</div>`:'<span class="muted">-</span>'}
function auditDetailHtml(detail){const value=parseAuditDetail(detail);const remaining={...value};delete remaining.deletionReason;const remainingText=Object.keys(remaining).length?JSON.stringify(remaining):'-';return `<div class="audit-detail" title="${escapeHtml(remainingText)}">${escapeHtml(remainingText)}</div>`}
  ...
  if(mode==='categories'){
    ...
    if(isAdmin){
      ...
      document.querySelectorAll('.delete-category').forEach(
        b=>b.onclick=()=>openDeleteCategoryConfirmation({id:b.dataset.id,name:b.dataset.name,count:Number(b.dataset.count||0)})
      );
    }
  }
  if(mode==='departments'){const r=await api('/api/admin/governance/departments');const isAdmin=isSystemAdmin();const headers=isAdmin?['รหัส','ชื่อหน่วยงาน','สถานะ','จัดการ']:['รหัส','ชื่อหน่วยงาน','สถานะ'];$('#departmentGovernanceTable').innerHTML=governanceTable(headers,r.data.map(x=>`<tr><td class="case-ref">${escapeHtml(x.code)}</td><td>${escapeHtml(x.name_th)}</td><td>${x.is_active?'<span class="v3-badge status-completed">เปิดใช้งาน</span>':'<span class="v3-badge status-cancelled">ปิดใช้งาน</span>'}</td>${isAdmin?`<td><button class="governance-edit edit-department" data-id="${x.id}" data-json='${escapeHtml(JSON.stringify(x))}'>แก้ไข</button></td>`:''}</tr>`));if(isAdmin)document.querySelectorAll('.edit-department').forEach(b=>b.onclick=()=>openGovernanceDialog('department',JSON.parse(b.dataset.json)))}
  if(mode==='users'){if(!isSystemAdmin()){throw new Error('เฉพาะผู้ดูแลระบบและผู้พัฒนาระบบเท่านั้นที่ดูข้อมูลผู้ใช้งานได้')}const r=await api('/api/admin/governance/users');const headers=['ชื่อผู้ใช้','ชื่อแสดงผล','สิทธิ์','หน่วยงาน','เข้าสู่ระบบล่าสุด','สถานะ','จัดการ'];$('#userGovernanceTable').innerHTML=governanceTable(headers,r.data.map(x=>{const protectedDev=x.role==='dev';const editDisabled=protectedDev&&currentUser.role!=='dev';return `<tr><td class="case-ref">${escapeHtml(x.username)}</td><td>${escapeHtml(x.display_name)}</td><td>${escapeHtml(roleLabel(x.role))}</td><td>${escapeHtml(x.department_name|| (['admin','dev','executive','exclusive'].includes(x.role)?'ทุกหน่วยงาน':'ยังไม่ได้กำหนด'))}</td><td>${fmt(x.last_login_at)}</td><td>${x.is_active?'<span class="v3-badge status-completed">ใช้งาน</span>':'<span class="v3-badge status-cancelled">ระงับ</span>'}</td><td><div class="governance-actions"><button class="governance-edit edit-user" data-json='${escapeHtml(JSON.stringify(x))}' ${editDisabled?'disabled title="Admin ไม่สามารถแก้ไขบัญชี DEV ได้"':''}>แก้ไข</button><button class="governance-delete delete-user" data-json='${escapeHtml(JSON.stringify(x))}' ${x.id===currentUser.id||protectedDev?'disabled title="ไม่สามารถลบบัญชี DEV หรือบัญชีที่กำลังใช้งาน"':''}>ลบ</button></div></td></tr>`}));document.querySelectorAll('.edit-user:not(:disabled)').forEach(b=>b.onclick=()=>openGovernanceDialog('user',JSON.parse(b.dataset.json)));document.querySelectorAll('.delete-user:not(:disabled)').forEach(b=>b.onclick=()=>openDeleteUserConfirmation(JSON.parse(b.dataset.json)))}
  if(mode==='staffProfiles'){if(currentUser?.role!=='dev'){throw new Error('เฉพาะ DEV เท่านั้นที่ดูข้อมูลเจ้าหน้าที่ได้')}const r=await api('/api/admin/governance/staff-profiles');const headers=['ชื่อ–สกุล','ตำแหน่ง','หน่วยงาน','LINE ID','เบอร์โทรศัพท์','แก้ไขล่าสุด','จัดการ'];$('#staffProfileGovernanceTable').innerHTML=governanceTable(headers,r.data.map(x=>`<tr><td><b>${escapeHtml(x.full_name)}</b></td><td>${escapeHtml(x.position_title)}</td><td>${escapeHtml(x.department_name||'ยังไม่กำหนด')}</td><td class="case-ref">${escapeHtml(x.line_id||'-')}</td><td>${escapeHtml(x.phone||'-')}</td><td>${fmt(x.updated_at)}</td><td><div class="governance-actions"><button class="governance-edit edit-staff-profile" data-json='${escapeHtml(JSON.stringify(x))}'>แก้ไข</button><button class="governance-delete delete-staff-profile" data-json='${escapeHtml(JSON.stringify(x))}'>ลบ</button></div></td></tr>`));document.querySelectorAll('.edit-staff-profile').forEach(b=>b.onclick=()=>openGovernanceDialog('staffProfile',JSON.parse(b.dataset.json)));document.querySelectorAll('.delete-staff-profile').forEach(b=>b.onclick=()=>openDeleteStaffProfileConfirmation(JSON.parse(b.dataset.json)))}
  if(mode==='audit'){const r=await api('/api/admin/governance/audit-logs');$('#auditGovernanceTable').innerHTML=governanceTable(['วันเวลา','ผู้ดำเนินการ','กิจกรรม','ประเภท','หมายเหตุ','รายละเอียด'],r.data.map(x=>`<tr><td>${fmt(x.created_at)}</td><td>${escapeHtml(x.actor_name||'ระบบ')}</td><td class="case-ref">${escapeHtml(x.action)}</td><td>${escapeHtml(x.entity_type)}</td><td>${auditNoteHtml(x.detail)}</td><td>${auditDetailHtml(x.detail)}</td></tr>`))}
}catch(e){show('#pageAlert',e.message)}}
...
function staffProfileDepartmentField(data=null){
  const selected=data?.department_id||'';
  const options=['<option value="">-- เลือกหน่วยงาน --</option>',...departments.filter(d=>d.is_active!==false).map(d=>`<option value="${d.id}" ${d.id===selected?'selected':''}>${escapeHtml(d.name_th)}</option>`)].join('');
  return `<label>หน่วยงาน<select name="departmentId" required>${options}</select></label>`;
}
function openGovernanceDialog(type,data=null){
  ...
  if((type==='department'||type==='category'||type==='user')&&!isSystemAdmin()){
  ...
  if(type==='category'){title.textContent=data?'แก้ไขหมวดหมู่':'เพิ่มหมวดหมู่';fields.innerHTML=dialogField('รหัสหมวดหมู่','code','text',data?.code||'','required minlength="2" maxlength="50" pattern="[A-Za-z0-9_]+" autocomplete="off"')+dialogField('ชื่อหมวดหมู่','nameTh','text',data?.name_th||'','required')+categoryDepartmentField(data)+dialogField('SLA (ชั่วโมง)','slaHours','number',data?.sla_hours||72,'required min="1"')+(data?`<label>สถานะ<select name="isActive"><option value="true" ${data.is_active?'selected':''}>เปิดใช้งาน</option><option value="false" ${!data.is_active?'selected':''}>ปิดใช้งาน</option></select></label>`:'')}
  if(type==='department'){title.textContent=data?'แก้ไขหน่วยงาน':'เพิ่มหน่วยงาน';fields.innerHTML=dialogField('รหัสหน่วยงาน','code','text',data?.code||'','required minlength="2" maxlength="50" pattern="[A-Za-z0-9_]+" autocomplete="off"')+dialogField('ชื่อหน่วยงาน','nameTh','text',data?.name_th||'','required')+(data?`<label>สถานะ<select name="isActive"><option value="true" ${data.is_active?'selected':''}>เปิดใช้งาน</option><option value="false" ${!data.is_active?'selected':''}>ปิดใช้งาน</option></select></label>`:'')}
  if(type==='user'){title.textContent=data?'แก้ไขผู้ใช้งาน':'เพิ่มผู้ใช้งาน';const devOption=currentUser?.role==='dev'&&data?.role==='dev'?'<option value="dev" selected>DEV</option>':'';fields.innerHTML=(data?'':dialogField('ชื่อผู้ใช้','username','text','','required'))+dialogField('ชื่อแสดงผล','displayName','text',data?.display_name||'','required')+`<label>สิทธิ์<select name="role" id="governanceUserRole"><option value="officer" ${data?.role==='officer'?'selected':''}>Officer</option><option value="supervisor" ${data?.role==='supervisor'?'selected':''}>Supervisor</option><option value="executive" ${['executive','exclusive'].includes(data?.role)?'selected':''}>Executive</option><option value="admin" ${data?.role==='admin'?'selected':''}>Admin</option>${devOption}</select></label>`+userDepartmentField(data)+dialogField(data?'รหัสผ่านใหม่ (เว้นว่างหากไม่เปลี่ยน)':'รหัสผ่านอย่างน้อย 12 ตัว','password','password','',''+(data?'':'required minlength="12"'))+(data?`<label>สถานะ<select name="isActive"><option value="true" ${data.is_active?'selected':''}>ใช้งาน</option><option value="false" ${!data.is_active?'selected':''}>ระงับ</option></select></label>`:'');const syncDepartmentField=()=>{const role=$('#governanceUserRole')?.value;const field=$('#userDepartmentField');if(field)field.classList.toggle('hidden',['admin','dev','executive'].includes(role))};$('#governanceUserRole').onchange=syncDepartmentField;syncDepartmentField()}
  if(type==='staffProfile'){title.textContent=data?'แก้ไขข้อมูลเจ้าหน้าที่':'เพิ่มข้อมูลเจ้าหน้าที่';fields.innerHTML=dialogField('ชื่อ–สกุล','fullName','text',data?.full_name||'','required maxlength="200" autocomplete="name"')+dialogField('ตำแหน่ง','positionTitle','text',data?.position_title||'','required maxlength="200"')+staffProfileDepartmentField(data)+dialogField('LINE ID (ไม่บังคับ)','lineId','text',data?.line_id||'','maxlength="100" autocomplete="off"')+dialogField('เบอร์โทรศัพท์ (ไม่บังคับ)','phone','tel',data?.phone||'','maxlength="30" inputmode="tel"')}
  $('#governanceDialog').showModal()}
async function saveGovernance(){
  if(governanceEditing?.type==='staffProfile'&&currentUser?.role!=='dev'){
    throw new Error('เฉพาะ DEV เท่านั้นที่จัดการข้อมูลเจ้าหน้าที่ได้');
  }
  if(['department','category','user'].includes(governanceEditing?.type)&&!isSystemAdmin()){
  ...
  if(type==='category'){if(!v.departmentId)throw new Error('กรุณาเลือกหน่วยงานรับผิดชอบ');path=data?`/api/admin/governance/categories/${data.id}`:'/api/admin/governance/categories';if(data)method='PATCH';payload={code:v.code.trim().toUpperCase(),nameTh:v.nameTh,departmentId:v.departmentId,slaHours:Number(v.slaHours),...(data?{isActive:v.isActive==='true'}:{})}}
  if(type==='department'){path=data?`/api/admin/governance/departments/${data.id}`:'/api/admin/governance/departments';if(data)method='PATCH';payload={code:v.code.trim().toUpperCase(),nameTh:v.nameTh,...(data?{isActive:v.isActive==='true'}:{})}}
  if(type==='user'){if(['officer','supervisor'].includes(v.role)&&!v.departmentId)throw new Error('กรุณาเลือกหน่วยงานสำหรับ Officer หรือ Supervisor');path=data?`/api/admin/governance/users/${data.id}`:'/api/admin/governance/users';if(data)method='PATCH';payload={displayName:v.displayName,role:v.role,departmentId:['admin','dev','executive'].includes(v.role)?null:v.departmentId,...(data?{isActive:v.isActive==='true',password:v.password||null}:{username:v.username,password:v.password})}}
  if(type==='staffProfile'){if(!v.departmentId)throw new Error('กรุณาเลือกหน่วยงาน');path=data?`/api/admin/governance/staff-profiles/${data.id}`:'/api/admin/governance/staff-profiles';if(data)method='PATCH';payload={fullName:v.fullName,positionTitle:v.positionTitle,departmentId:v.departmentId,lineId:v.lineId,phone:v.phone}}
...
function openDeleteCategoryConfirmation(category){categoryPendingDelete=category;clear('#deleteCategoryAlert');$('#deleteCategoryName').textContent=category.name;const warning=$('#deleteCategoryUsageWarning');warning.textContent=category.count>0?`หมวดหมู่นี้มีเรื่องร้องเรียนใช้งานอยู่ ${category.count} เรื่อง จึงไม่สามารถลบได้ กรุณาปิดใช้งานแทน`:'หมวดหมู่นี้ยังไม่มีเรื่องร้องเรียนใช้งานและสามารถลบได้';warning.classList.toggle('danger-text',category.count>0);const button=$('#deleteCategoryConfirmButton');button.disabled=category.count>0;button.textContent='ลบหมวดหมู่';const dialog=$('#deleteCategoryDialog');if(!dialog.open)dialog.showModal()}
function closeDeleteCategoryConfirmation(){const dialog=$('#deleteCategoryDialog');if(dialog.open)dialog.close('cancel');categoryPendingDelete=null;clear('#deleteCategoryAlert')}
async function deleteCategory(){if(!categoryPendingDelete)return;const pending={...categoryPendingDelete};const button=$('#deleteCategoryConfirmButton');button.disabled=true;button.textContent='กำลังลบ…';clear('#deleteCategoryAlert');try{await api(`/api/admin/governance/categories/${pending.id}`,{method:'DELETE'});closeDeleteCategoryConfirmation();show('#pageAlert',`ลบหมวดหมู่ ${pending.name} เรียบร้อย`,'success');await loadGovernance('categories')}catch(error){show('#deleteCategoryAlert',error.message);button.disabled=false;button.textContent='ลบหมวดหมู่'}}
...
categoryDeleteDialogElement.addEventListener('cancel',e=>{e.preventDefault();closeDeleteCategoryConfirmation()});
categoryDeleteDialogElement.addEventListener('click',e=>{if(isDialogBackdropClick(e,categoryDeleteDialogElement))closeDeleteCategoryConfirmation()});
let userPendingDelete=null;
function openDeleteUserConfirmation(user){userPendingDelete={id:user.id,username:user.username,displayName:user.display_name};clear('#deleteUserAlert');$('#deleteUserName').textContent=user.display_name;$('#deleteUserUsername').textContent=user.username;const button=$('#deleteUserConfirmButton');button.disabled=false;button.textContent='ลบผู้ใช้งาน';const dialog=$('#deleteUserDialog');if(!dialog.open)dialog.showModal()}
function closeDeleteUserConfirmation(){const dialog=$('#deleteUserDialog');if(dialog.open)dialog.close('cancel');userPendingDelete=null;clear('#deleteUserAlert')}
async function deleteUser(){if(!userPendingDelete)return;const pending={...userPendingDelete};const button=$('#deleteUserConfirmButton');button.disabled=true;button.textContent='กำลังลบ…';clear('#deleteUserAlert');try{await api(`/api/admin/governance/users/${pending.id}`,{method:'DELETE'});closeDeleteUserConfirmation();show('#pageAlert',`ลบผู้ใช้งาน ${pending.username} เรียบร้อย`,'success');await loadGovernance('users')}catch(error){show('#deleteUserAlert',error.message);button.disabled=false;button.textContent='ลบผู้ใช้งาน'}}
...
userDeleteDialogElement.addEventListener('cancel',e=>{e.preventDefault();closeDeleteUserConfirmation()});
userDeleteDialogElement.addEventListener('click',e=>{if(isDialogBackdropClick(e,userDeleteDialogElement))closeDeleteUserConfirmation()});
let staffProfilePendingDelete=null;
function openDeleteStaffProfileConfirmation(profile){staffProfilePendingDelete={id:profile.id,fullName:profile.full_name};clear('#deleteStaffProfileAlert');$('#deleteStaffProfileName').textContent=profile.full_name;const button=$('#deleteStaffProfileConfirmButton');button.disabled=false;button.textContent='ลบข้อมูลเจ้าหน้าที่';const dialog=$('#deleteStaffProfileDialog');if(!dialog.open)dialog.showModal()}
function closeDeleteStaffProfileConfirmation(){const dialog=$('#deleteStaffProfileDialog');if(dialog.open)dialog.close('cancel');staffProfilePendingDelete=null;clear('#deleteStaffProfileAlert')}
async function deleteStaffProfile(){if(!staffProfilePendingDelete)return;const pending={...staffProfilePendingDelete};const button=$('#deleteStaffProfileConfirmButton');button.disabled=true;button.textContent='กำลังลบ…';clear('#deleteStaffProfileAlert');try{await api(`/api/admin/governance/staff-profiles/${pending.id}`,{method:'DELETE'});closeDeleteStaffProfileConfirmation();show('#pageAlert',`ลบข้อมูลเจ้าหน้าที่ ${pending.fullName} เรียบร้อย`,'success');await loadGovernance('staffProfiles')}catch(error){show('#deleteStaffProfileAlert',error.message);button.disabled=false;button.textContent='ลบข้อมูลเจ้าหน้าที่'}}
const staffProfileDeleteDialogElement=$('#deleteStaffProfileDialog');
$('#deleteStaffProfileCancelButton').onclick=closeDeleteStaffProfileConfirmation;
$('#deleteStaffProfileConfirmButton').onclick=deleteStaffProfile;
staffProfileDeleteDialogElement.addEventListener('cancel',e=>{e.preventDefault();closeDeleteStaffProfileConfirmation()});
staffProfileDeleteDialogElement.addEventListener('click',e=>{if(isDialogBackdropClick(e,staffProfileDeleteDialogElement))closeDeleteStaffProfileConfirmation()});
