...
function openGovernanceDialog(type,data=null){
  ...
  if(type==='staffProfile'){title.textContent=data?'แก้ไขข้อมูลเจ้าหน้าที่':'เพิ่มข้อมูลเจ้าหน้าที่';fields.innerHTML=dialogField('ชื่อ–สกุล','fullName','text',data?.full_name||'','required maxlength="200" autocomplete="name"')+dialogField('ตำแหน่ง','positionTitle','text',data?.position_title||'','required maxlength="200"')+staffProfileDepartmentField(data)+dialogField('LINE ID','lineId','text',data?.line_id||'','required maxlength="100" autocomplete="off"')+dialogField('เบอร์โทรศัพท์','phone','tel',data?.phone||'','required minlength="9" maxlength="30" inputmode="tel"')}
