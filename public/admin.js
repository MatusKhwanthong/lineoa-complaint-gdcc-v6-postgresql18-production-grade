...
function renderBars(sel,data=[],suffix='',showPercentage=false){const values=data.map(x=>Math.max(0,Number(x.value)||0));const max=Math.max(1,...values);const sum=values.reduce((total,value)=>total+value,0);const scale=showPercentage?Math.max(1,sum):max;$(sel).innerHTML=data.map((x,i)=>{const value=values[i];const percentage=sum?value/sum*100:0;const percentageLabel=percentage.toLocaleString('th-TH',{minimumFractionDigits:percentage%1===0?0:1,maximumFractionDigits:1});const valueLabel=`${x.value}${suffix?` ${suffix}`:''}${showPercentage?` (${percentageLabel}%)`:''}`;return `<div class="metric-item ${showPercentage?'insight-metric-item':''}"><div class="metric-label"><span class="metric-name">${showPercentage?`<i class="metric-rank">${i+1}</i>`:''}${escapeHtml(x.label)}</span><b class="metric-value">${escapeHtml(valueLabel)}</b></div><progress class="metric-progress metric-tone-${i%6}" value="${value}" max="${scale}" aria-label="${escapeHtml(x.label)} ${escapeHtml(valueLabel)}"></progress></div>`}).join('')||'<p class="muted">ยังไม่มีข้อมูล</p>'}
...
function renderReports(){const s=dashboardCache.summary;const data=[['เรื่องทั้งหมด',s.total],['เดือนนี้',s.this_month],['กำลังดำเนินการ',s.in_progress],['เสร็จสิ้น',s.completed],['เฉลี่ยวันดำเนินการ',s.avg_days]];$('#reportSummary').innerHTML=data.map(([l,v])=>`<div><strong>${v||0}</strong><span>${l}</span></div>`).join('');renderBars('#reportCategory',dashboardCache.categoryBreakdown,'',true);renderBars('#reportDepartment',dashboardCache.departmentBreakdown,'',true);renderTrend('#reportTrend',dashboardCache.monthlyTrend)}
