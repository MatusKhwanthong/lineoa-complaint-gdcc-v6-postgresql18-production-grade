...
function renderBars(sel,data=[],suffix='',showPercentage=false){const values=data.map(x=>Number(x.value)||0);const max=Math.max(1,...values);const total=Math.max(1,values.reduce((sum,value)=>sum+value,0));$(sel).innerHTML=data.map((x,i)=>{const value=values[i];const percentage=value/total*100;const width=showPercentage?percentage:value/max*100;const percentageLabel=percentage.toLocaleString('th-TH',{minimumFractionDigits:percentage%1===0?0:1,maximumFractionDigits:1});return `<div><div class="metric-label"><span>${escapeHtml(x.label)}</span><b>${escapeHtml(x.value)}${suffix?` ${escapeHtml(suffix)}`:''}${showPercentage?` (${percentageLabel}%)`:''}</b></div><div class="metric-track"><div class="metric-fill" style="width:${width}%;filter:hue-rotate(${i*8}deg)"></div></div></div>`}).join('')||'<p class="muted">ยังไม่มีข้อมูล</p>'}
