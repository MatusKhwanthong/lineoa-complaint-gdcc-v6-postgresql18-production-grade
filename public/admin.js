...
function tableHtml(rows,compact=false){return `<div class="table-wrap"><table class="v3-table"><thead><tr><th>เลขรับเรื่อง</th><th>เรื่องร้องเรียน</th>${compact?'':'<th>ผู้ร้อง</th>'}<th>หมวดหมู่</th><th>หน่วยงาน</th><th>วันที่รับเรื่อง</th><th>ความสำคัญ</th><th>สถานะ</th><th></th></tr></thead><tbody>${rows.map(r=>`<tr><td class="case-ref">${r.reference_no}</td><td><div class="case-title">${escapeHtml(r.title)}</div></td>${compact?'':`<td>${escapeHtml(r.contact_name||'-')}</td>`}<td>${escapeHtml(r.category_name||'-')}</td><td>${escapeHtml(r.department_name||'ยังไม่มอบหมาย')}</td><td>${fmt(r.created_at)}</td><td>${priorityBadge(r.priority)}</td><td>${badge(r.status)}</td><td><button class="view-case" data-id="${r.id}">ดูรายละเอียด</button></td></tr>`).join('')}</tbody></table></div>`}
