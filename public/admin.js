...
function renderDashboard(){renderKpis();renderTrend('#trendChart',dashboardCache.monthlyTrend);renderDonut();renderUserUsage();renderUrgent();if(currentUser?.role!=='officer'){renderBars('#categoryBars',dashboardCache.categoryBreakdown);renderBars('#departmentBars',dashboardCache.departmentBreakdown)}$('#recentTable').innerHTML=tableHtml(dashboardCache.recent,true)}
...
function renderPagedInsight(sel,pagerSel,data=[],pageKey){const totalPages=data.length>1?2:1;const pageSize=Math.max(1,Math.ceil(data.length/totalPages));const page=Math.min(totalPages,Math.max(1,reportInsightPages[pageKey]||1));reportInsightPages[pageKey]=page;const start=(page-1)*pageSize;renderBars(sel,data.slice(start,start+pageSize),'',true,{totalData:data,rankOffset:start});const pager=$(pagerSel);pager.innerHTML=Array.from({length:totalPages},(_,index)=>`<button type="button" class="insight-page-button ${page===index+1?'active':''}" data-page="${index+1}" aria-label="หน้าที่ ${index+1}" ${page===index+1?'aria-current="page"':''}>${index+1}</button>`).join('');pager.querySelectorAll('.insight-page-button').forEach(button=>button.onclick=()=>{reportInsightPages[pageKey]=Number(button.dataset.page);renderPagedInsight(sel,pagerSel,data,pageKey)})}
