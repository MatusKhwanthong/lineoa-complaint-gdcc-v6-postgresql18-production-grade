...
function auditDetailHtml(detail){let value=detail;if(typeof value==='string'){try{value=JSON.parse(value)}catch{return `<div class="audit-detail" title="${escapeHtml(value)}">${escapeHtml(value)}</div>`}}if(!value||typeof value!=='object')return '<div class="audit-detail">-</div>';const deletionReason=String(value.deletionReason||'').trim();const remaining={...value};delete remaining.deletionReason;const remainingText=Object.keys(remaining).length?JSON.stringify(remaining):'';return `${deletionReason?`<div class="audit-delete-note"><strong>หมายเหตุการลบ:</strong> ${escapeHtml(deletionReason)}</div>`:''}${remainingText?`<div class="audit-detail" title="${escapeHtml(remainingText)}">${escapeHtml(remainingText)}</div>`:''}`||'<div class="audit-detail">-</div>'}
