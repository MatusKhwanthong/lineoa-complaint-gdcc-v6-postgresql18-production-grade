...
async function loadGovernance(mode='categories'){setGovernanceTab(mode);try{
  ...
  if(mode==='staffProfiles'){if(currentUser?.role!=='dev'){throw new Error('เฉพาะ DEV เท่านั้นที่ดูข้อมูลเจ้าหน้าที่ได้')}const r=await api('/api/admin/governance/staff-profiles');const headers=['ชื่อ–สกุล','ตำแหน่ง','LINE ID','เบอร์โทรศัพท์','แก้ไขล่าสุด','จัดการ'];$('#staffProfileGovernanceTable').innerHTML=governanceTable(headers,r.data.map(x=>`<tr><td><b>${escapeHtml(x.full_name)}</b></td><td>${escapeHtml(x.position_title)}</td><td class="case-ref">${escapeHtml(x.line_id)}</td><td>${escapeHtml(x.phone)}</td><td>${fmt(x.updated_at)}</td><td><button class="governance-edit edit-staff-profile" data-json='${escapeHtml(JSON.stringify(x))}'>แก้ไข</button></td></tr>`));document.querySelectorAll('.edit-staff-profile').forEach(b=>b.onclick=()=>openGovernanceDialog('staffProfile',JSON.parse(b.dataset.json)))}
...
function openGovernanceDialog(type,data=null){
  ...
  if(type==='staffProfile'){title.textContent=data?'แก้ไขข้อมูลเจ้าหน้าที่':'เพิ่มข้อมูลเจ้าหน้าที่';fields.innerHTML=dialogField('ชื่อ–สกุล','fullName','text',data?.full_name||'','required maxlength="200" autocomplete="name"')+dialogField('ตำแหน่ง','positionTitle','text',data?.position_title||'','required maxlength="200"')+dialogField('LINE ID','lineId','text',data?.line_id||'','required maxlength="100" autocomplete="off"')+dialogField('เบอร์โทรศัพท์','phone','tel',data?.phone||'','required minlength="9" maxlength="30" inputmode="tel"')}
...
async function saveGovernance(){
  ...
  if(type==='staffProfile'){path=data?`/api/admin/governance/staff-profiles/${data.id}`:'/api/admin/governance/staff-profiles';if(data)method='PATCH';payload={fullName:v.fullName,positionTitle:v.positionTitle,lineId:v.lineId,phone:v.phone}}
