...
function appView(user){currentUser=user;const isOfficer=user.role==='officer';$('#loginPanel').classList.add('hidden');$('#appShell').classList.remove('hidden');$('#staffName').textContent=user.displayName||user.username;$('#staffRole').textContent=roleLabel(user.role);$('#todayLabel').textContent=new Intl.DateTimeFormat('th-TH',{dateStyle:'full'}).format(new Date());$('#topCategoriesCard').classList.toggle('hidden',isOfficer);$('#departmentWorkloadCard').classList.toggle('hidden',isOfficer)}
